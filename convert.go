...
func (c *Convert) SegmentFilename(home string) string {
	return fmt.Sprintf(`%s-%%d.%s`, filepath.Join(c.DestDir(home), gfx.Name(c.File.Name)), hlsSegmentExt)
}
...
func (c *Convert) DestFilename(home string) (filename string) {
	dir := c.DestDir(home)
	if nil != c.Hls {
		filename = filepath.Join(dir, gfx.Name(c.File.Name, gfx.Ext(hlsPlaylistExt)))
	} else if media.Type_AUDIO == c.Type {
		filename = filepath.Join(dir, gfx.Name(c.File.Name, gfx.Ext(c.Audio.Format.String())))
	} else if media.Type_VIDEO == c.Type {
		filename = filepath.Join(dir, gfx.Name(c.File.Name, gfx.Ext(c.Video.Format.String())))
	}
